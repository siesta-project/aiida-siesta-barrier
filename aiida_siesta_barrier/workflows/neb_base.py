...
#from aiida_siesta_barrier.utils.xyz_utils import write_xyz_file_from_structure
#from aiida_siesta_barrier.utils.structures import add_ghost_sites_to_structure
from aiida_siesta.calculations.siesta import SiestaCalculation

@calcfunction
def parse_traj(neb_remote_folder, ref_structure):
    """
    ...
    folder_path = neb_remote_folder.get_remote_path()
...
class SiestaBaseNEBWorkChain(WorkChain):
    ...
    def run_results(self):
        ...
        if not self.ctx.neb_wk.is_finished_ok:
            ...
            remote_folder_node = load_node(self.ctx.neb_wk.pk)
            annotated_traj = parse_traj(remote_folder_node.outputs.remote_folder,ref_structure)
            self.out('neb_output_package',annotated_traj)
            self.report('NEB calculations tarj saved though')
